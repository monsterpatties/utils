package com.monsterPatties.networking 
{
	/**
	 * ...
	...
	import sample.ui.InfoBox;
	import sample.ui.Prompt
	import sample.ui.Lobby
	 
	 
	public class Conn extends Sprite
	{
		/*------------------------------------------------------------------------------------Constant------------------------------------------------------*/
		...
		//new
		private var _spectatorMC:spectatingMC;
		private var _player1MC:Player1MC;
		private var _player2MC:Player2MC;
		private var _showTurnMC:ShowTurnMC;		
		/*------------------------------------------------------------------------------------Constructor---------------------------------------------------*/
		
		
		
		public function Conn( stage:* ) 
		{
			_spectatorMC = new spectatingMC();
			addChild(  _spectatorMC );
			
			_player1MC = new Player1MC();
			addChild( _player1MC );
			
			_player2MC = new Player2MC();
			addChild( _player2MC );
			
			_showTurnMC = new ShowTurnMC();
			addChild( _showTurnMC );
		...
		{
			new Prompt(_stage, "What's your name?", "Guest-" + (Math.random()*9999<<0), function(name:String){
				PlayerIO.connect(
					_stage,								//Referance to _stage
					"tictactoe-eibfdezyjke7giutgh6vsg",			//Game id (Get your own at playerio.com. 1: Create user, 2:Goto admin pannel, 3:Create game, 4: Copy game id inside the "")
		...
		private function handleJoin(connection:Connection):void{
			...
			//Add chat to game
			var chat:Chat = new Chat(_stage, connection);			
			
			connection.addMessageHandler("init", function(m:Message, iAm:int, name:String){
				isSpectator = false
				_spectatorMC.visible = false
				imPlayer = iAm;
				(imPlayer == 0 ? _player1MC.player1 : _player2MC.player2).text  = name;  
			})
			
			
			connection.addMessageHandler("join", function(m:Message, p1name:String, p2name:String){
				_player1MC.player1.text = p1name;
				_player2MC.player2.text = p2name;
			})
		...
		private function setTurn(turn:Boolean, clear:Boolean = false):void{
			...
			_showTurnMC.gotoAndStop( imPlayer == 0 ? turn ? 1 : 2 : turn ? 2 : 1 );
